...
(async () => {
	...
	const { modules, dependencies } = JSON.parse(
		execSync(
			`${process.env.CLI_PATH!} move build --dump-bytecode-as-base64 --path ${process.env.EXTENSIONS_PATH!} ${process.env.NETWORK === "testnet" && "--dev"}`,
			{ encoding: 'utf-8' }
		)
	);
	...
	try {
		...
		fs.writeFileSync('./src/data/extensions-created.json', JSON.stringify(objects, null, 2));
		...
		execSync(
			`${process.env.CLI_PATH!} move manage-package --environment "$(sui client active-env)" \
				...
				--original-id ${getId("extensions-package")} \
				--latest-id ${getId("extensions-package")} \
				--version-number '1' \
				--path ${process.env.EXTENSIONS_PATH!}`,
			{ encoding: 'utf-8' }
		)
	};
})()