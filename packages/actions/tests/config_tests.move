...
//     let mut proposal = create_dummy_proposal(&mut scenario, &mut account, &extensions);  config::new_config_deps(&mut proposal, &account, &extensions, vector[b"External".to_string()], vector[@0xABC], vector[1], DummyIntent());
//     account.add_proposal(proposal, version::current(), DummyIntent());
