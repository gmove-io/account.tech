...
/// Constructs an auth from a Witness, an (optional) name and a multisig id
...
/// Asserts that the auth has been issued from kraken (multisig or actions) packages
...
/// Asserts that the auth has been issued from kraken core (multisig or actions) packages
...
/// Verifies that the action is executed for the multisig that approved it
